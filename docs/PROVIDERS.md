...
Dollar-budget enforcement is opt-in (`agent.max_cost_usd = 0` by default). With `agent.max_cost_usd > 0`, each network request requires usable input/output pricing for its resolved model, including summarizers and server-side fallback candidates. The runtime reserves estimated request cost and holds observed attempt charges in the shared parent budget until settlement. Concurrent helpers, retries, summaries, and fallback routes check both amounts before dispatch. Terminal usage remains held across cancellation until it reaches the accounting sink. Chat Completions reasoning-disabled recovery and Responses reasoning-summary recovery report each wire attempt through the same observer before checking the remaining budget and dispatching recovery. Aggregate responses are not observed a second time. If an OpenAI-compatible, Anthropic, or Gemini stream stops after reporting usage, its latest cumulative snapshot is retained with incomplete status. Chat Completions error frames and HTTP error bodies retain their reported billing usage before raising; failed responses never authorize tool execution. Context-capacity failures can select another route, including one with the same context window and a smaller output reservation; global cost, time, and request limits remain terminal. Missing pricing or unreconciled usage stops the run with a budget error. These estimates cannot guarantee the final provider bill. Set `max_cost_usd = 0` to disable the dollar gate while retaining time, request, and context limits.
