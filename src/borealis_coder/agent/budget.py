...
class Budget:
    ...
    def before_turn(self) -> None:
        ...
        if self.elapsed_seconds >= self.config.max_time_seconds:
            raise BudgetExceeded("time", f"Maximum {self.config.max_time_seconds}s run time reached")
        assert self.usage is not None
        if self.config.max_cost_usd > 0 and self.usage.cost_usd >= self.config.max_cost_usd:
            raise BudgetExceeded("cost", f"Maximum ${self.config.max_cost_usd:.2f} model cost reached")
        self.turns += 1
    ...
    def add_usage(self, usage: Usage) -> None:
        ...
        if self.config.max_cost_usd > 0 and self.usage.cost_usd > self.config.max_cost_usd:
            raise BudgetExceeded("cost", f"Model cost ${self.usage.cost_usd:.4f} exceeded ${self.config.max_cost_usd:.2f}")


def estimate_request_tokens(system: str, messages: list[Message], tools: list[dict]) -> int:  # type: ignore[type-arg]
    chars = len(system) + len(json_dumps(tools))
    chars += sum(len(message.content) + len(json_dumps([call.to_dict() for call in message.tool_calls])) for message in messages)
    # Add framing overhead per message and tool.
    return estimate_tokens("x" * chars) + len(messages) * 12 + len(tools) * 30
