...
class IgnoreMatcher:
    def __init__(self, root: Path, *, ignored_dirs: list[str] | None = None) -> None:
        ...
        for name in (".gitignore", ".borealisignore"):
            path = self.root / name
            if path.is_file():
                self.rules.extend(_parse_rules(path.read_text(encoding="utf-8", errors="replace")))
    ...
    def ignored(self, path: Path, *, is_dir: bool | None = None) -> bool:
        ...
        for rule in self.rules:
            ...
            match = False
            if rule.anchored:
                match = fnmatch.fnmatch(relative, pattern)
            elif "/" in pattern:
                match = fnmatch.fnmatch(relative, pattern) or fnmatch.fnmatch(relative, f"**/{pattern}")
            else:
                match = any(fnmatch.fnmatch(part, pattern) for part in parts)
            if match:
                state = not rule.negate
        return state


def repository_files(root: Path, matcher: IgnoreMatcher, *, include_untracked: bool = True) -> list[Path]:
    """Prefer git's ignore engine, then fall back to a bounded filesystem walk."""
    root = root.resolve()
    if (root / ".git").exists():
        args = [
            "git", "-c", "core.fsmonitor=false", "-c", "core.hooksPath=/dev/null",
            "-C", str(root), "ls-files", "--cached",
        ]
    ...
    for current, dirs, files in os.walk(root):
        ...
        dirs[:] = sorted(item for item in dirs if not matcher.ignored(current_path / item, is_dir=True))
